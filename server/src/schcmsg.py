...
def get_mic_size(rule):
    frag = rule["fragmentation"]
    ModProfil = frag["FRModeProfile"]
    MICAlgorithm = ModProfil["MICAlgorithm"]
    assert MICAlgorithm == "crc32"
    return 32
...
class frag_tx(frag_base):
    ...
    def make_frag(self, dtag, win=None, fcn=None, mic=None, bitmap=None,
        ...
        WSize = Mode["WSize"]
        '''print('******RULE:')
        print('self.rule["ruleLength"]: ', self.rule["ruleLength"])
        print('self.rule["ruleID"]: ', self.rule["ruleID"])
        print('Dtagsize: ', Dtagsize)
        print('FCNSize: ', FCNSize)
        print('MICAlgorithm: ', MICAlgorithm)
        print('WSize: ', WSize)'''
        ...
        if abort == True:
            ...
            if mic is not None and MICAlgorithm is not None:
...
class frag_receiver_tx(frag_base):
    ...
    def make_frag(self, dtag, win=None, cbit=None, bitmap=None, abort=False):
        ...
        if abort == True:
            ...
            # c-bit
            buffer.set_bit(1)
            padding_size = (self.rule["profile"].get("L2WordSize") -
                            buffer.count_added_bits()%self.rule["profile"].get("L2WordSize"))
            padding_size += self.rule["profile"].get("L2WordSize")
            # padding bits
...
class frag_rx(frag_base):
    ...
    def parse_fcn(self):
        ...
        print("----------------------[SCHC MSG]-----------------------")
        print("FCN in the packet: ",self.fcn)
        print("------------------------------------------------------")
        print()
        return FCNSize
...
class frag_receiver_rx(frag_rx):
    ...
    def __init__(self, rule, packet_bbuf):
        ...
        if self.fcn == get_fcn_all_1(self.rule):
            ...
            if self.packet_bbuf.count_remaining_bits() < L2WordSize:
