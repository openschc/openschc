...
class SCHC_server:
    def __init__(self):
        ...
        self.rule_fragin2 = "example/frag-rule-201.json"      #NoACK #RuleID = 4
        self.rule_fragout2 = "example/frag-rule-202.json"     #NoACK
        self.rule_fragin = "example/frag-rule-101.json"      #ACK-ON-Error RuleID = 2
...
#simul
        ...
        if self.loss_config is not None:
            self.simul_config["loss"] = self.loss_config
        self.sim = simul.Simul(self.simul_config)

        self.layer_2 = SimulLayer2(self.sim,self.port,self.devEUI)
        self.layer_2.set_mtu(self.l2_mtu)
        self.layer_3 = SimulLayer3(self.sim)
        self.scheduler = Scheduler()
        self.scheduler.run()
        self.protocol = schc.SCHCProtocol(self.scheduler,self.sim, self.layer_2, self.layer_3)
...
#-------------------------------------------------------------------------
    ...
    def reassemble(self,Fragment):
        ...
        print()
        print("----------------------[SCHS SERVER]-----------------------")
        print("FRAGMENT ",self.cont)
        print("------------------------------------------------------")
        
        answer = self.layer_2.event_receive_packet(self.sender_id, Fragment)
        return answer
