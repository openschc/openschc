...
def make_bit_list(tile_list, N, window_size):
    ...
    bit_list = {}
    wni = 0
    sorted_tile_list = sort_tile_list(tile_list, N) #sort the list into the FCN order.[ 6, 5, 4, 3, 2, 1, 0, 7 ]
    # main
    tni = max_fcn
    print()
    print("max_fcn", max_fcn)
    print()
    for t in sorted_tile_list:
        ...
        print("tile ",t)
        print()
        print("w-num ",wn, "wni ",wni)
        print()
        print("t-num", tn, "nb_tiles", nbt)
        print()
        # all-1
        if tn == all1(N):
            print("all 1 ")
            while tni > 0: #starts in 6
                bl.append(0)
                tni -= 1
            print("bit_list ",bl)
            if nbt == 1:
                ...
                break
        # regular
        if wni < wn: #quand il manque une fenetre
