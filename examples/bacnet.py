...
sequence1 = rdpcap("Test_bacnet_simulator.pcap")
...
for pkt in sequence:
    if pkt.haslayer(IP):
        ...
        a, b, c = parse.parse(pkt=bytes(pkt)[14:], layers=["IPv4", "UDP", "BACNET"], direction=dir, start="IPv4")
        r, d = rm.FindRuleFromPacket(a, direction=dir, failed_field=False)
        print (a, b, c)
        #print(d)
        if r:
            schc_pkt = compressor.compress(r, a, b, direction=dir)
            #print (len(schc_pkt._content))
            #print(len(bytes(pkt)[14:]))