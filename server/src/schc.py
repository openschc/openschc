...
class SCHCProtocol:
    ...
    def __init__(self, scheduler, system, layer2, layer3):
        #self.config = config
        self.system = system
    ...
    def event_receive_from_L2(self, sender_L2addr, raw_packet):
        ...
        print("------------------------------------------------------")
        if key == "fragSender" or key == "fragSender2":
            if rule["dtagSize"] > 0:
        ...
        elif key == "fragReceiver" or key == "fragReceiver2":
            print()
            print("----------------------Porotocol-----------------------")
            print("rule: ")
            print(rule)
            ...
            schc_packet, ACK_flag, ACK = session.receive_frag(packet_bbuf, dtag)
            if schc_packet != None:             
