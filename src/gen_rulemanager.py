...
FIELD__SCHC_HEADER = {
    T_SCHC_NXT            : {"FL": 4,  "TYPE": int, "ALGO": "DIRECT" }
}
...
class RuleManager:
            ...
            if not overlap:
                if T_COMP in n_rule:
                    ...
                    if already_exists is None:
                        ...
                        print("SCHC Header already defined, ignored.")
    ...
    def _create_compression_rule (self, nrule, device_id = None, schc_header=False):
        """
        ...
        for r in nrule[KEY]:
            if r["FID"] == T_COAP_OPT_END:
                ...
                continue
            if not r["FID"].upper() in FIELD_ALLOWED:
            ...
            entry = {}
            FID = r[T_FID].upper()
            if schc_header and FID.find("SCHC") != 0:
                raise ValueError("SCHC Header contains only SCHC FID.")
            entry[T_FID] = FID
            entry[T_FL] = self._return_default(r, T_FL, FIELD_ALLOWED[FID][T_FL])
...
# Find rules 
    ...
    def FindSCHCHeaderRule(self, deviceID=None):
        for d in self._ctxt:
            if d["DeviceID"] == deviceID:
                for r in d["SoR"]:
                    if T_SCHC_HEADER in r:
                        return r
